            ...
            <div class="col">
                <select id="grouping_parameters" class="form-control" name="grouping_parameters[]" onchange="changeVariations();">
                    @foreach($sCommerce->getProductOptionalsSelect((int)request()->i) as $optionalsSelect)
                        <option value="{{$optionalsSelect->filter}}" @if(in_array($optionalsSelect->filter, $product->grouping_parameters_array)) selected @endif>{{$optionalsSelect->pagetitle}}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
<div class="table-responsive">
    <table id="variations" class="table table-condensed table-hover sectionTrans">
        ...
        <tr>
            ...
            <th style="text-align:center;">{{$_lang["scommerce_code"]}}</th>
            ...
            @if((int)$filterId)
                @foreach($sCommerce->filterValues($filterId) as $value)
                    <tr>
                        ...
                        <td>
                            <div class="input-group">
                                <input type="text" name="variations[{{$value->vid}}][code]" class="form-control" value="{{$product->variations_array[$value->vid]['code'] ?? $product->code}}" onchange="documentDirty=true;">
                            </div>
                        </td>
                        <td>
                            <input type="hidden" name="variations[{{$value->vid}}][published]" value="0">
